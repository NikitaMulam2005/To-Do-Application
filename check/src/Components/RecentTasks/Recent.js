...
const Recent = ({list,loading,handleChange,handleDelete}) => {
  const updateList=list.slice().sort((a,b)=> new Date(a.date)- new Date(b.date))
  return (
      ...
      <h2>Add Task</h2></div></Link>
       ...
       {<div>
        ...
        updateList && updateList.length >0?(
              <div key={updateList._id} style={{paddingRight:'20px',paddingLeft:'20px'}}>
                {updateList.map((list)=>(
                    <div>
